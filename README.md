...
本專題使用 ESP32 觸發分類任務，Raspberry Pi / ArmPi-FPV 負責拍照與機械手臂控制，AI Server 負責 YOLOv8 影像辨識、分類決策與 high-level command 產生。系統模組之間使用 HTTP API 溝通；AI Server 不直接控制 ROS 或機械手臂，Raspberry Pi 收到 AI Server 回傳的 command 後再轉成 ArmPi-FPV motion plan 執行。
...
ESP32 偵測到觸發訊號
  ...
  -> Raspberry Pi 上傳影像到 AI Server /detect
  -> AI Server 執行 YOLOv8 推論
  -> AI Server 輸出 label、bbox、confidence、center
  -> AI Server 依 label 對應 target_bin
  -> AI Server 依 center 產生 pick_zone 或 workspace_candidate
  -> AI Server 回傳 high-level command
...
| `raspberry-pi/` | 拍照、呼叫 AI Server、驗證 command、產生 motion_plan、呼叫 ROS / ArmPi 控制機械手臂 | 不執行 YOLO，不自行決定分類結果 |
| `ai-server/` | AI Server 模組：提供 YOLO 推論 API、輸出 bbox/center、決定 label/bin、產生 high-level command | 不直接呼叫 ROS，不操作機械手臂 |
...
YOLO 權重輸出的 class name 必須與 `tissue`、`foil_pack`、`plastic` 完全一致；其他 label 由 AI Server 以 `422` 回應。
...
| 狀況 | HTTP |
| --- | --- |
| command、AI Server 回應或設定資料不符合合約 | `422` |
| AI Server 連線、ROS / ArmPi 執行或服務內部例外 | `500` |

## Command 合約

AI Server 回傳給 Raspberry Pi 的 `command` 必須符合：
...
`PICK_POINTS` 負責把 `left`、`middle`、`right` 對到手臂夾取點；`BIN_POINTS` 負責把 `bin_a`、`bin_b`、`bin_c` 對到分類桶位置；`WORKSPACE_LIMITS` 負責限制 AI Server 給的工作區座標。

## AI Server 設定

倉庫只保留 `ai-server/config.example.py`。本機執行時複製成不提交的 `ai-server/config.py`。
...
- 專案維持三個系統模組：`esp32/`、`raspberry-pi/`、`ai-server/`。
- AI Server 只回傳 high-level command；motion plan 與 ROS / ArmPi 控制由 Raspberry Pi 端實作。
- 模組之間使用 HTTP API 溝通，不共用本機程式碼。
- 不要提交真實 API key、WiFi 密碼、本機設定檔、YOLO 權重或原始資料集。
- 不要提交 `ai-server/config.py`、`esp32/config.h`、`raspberry-pi/config.py`、YOLO 權重或原始資料集。
